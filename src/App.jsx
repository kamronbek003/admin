...
function App() {
    ...
    // Initialize activeSection from localStorage, defaulting to 'dashboard'
    const [activeSection, setActiveSection] = useState(() => {
        const savedSection = localStorage.getItem('active_section_key');
        // Check if the saved section is a valid key in our sections object
        if (savedSection && sections.hasOwnProperty(savedSection)) {
            return savedSection;
        }
        return 'dashboard'; // Default value
    });
    ...
    // Applications Notification State
    ...
    useEffect(() => {
        if (sections.hasOwnProperty(activeSection)) {
            ...
            // Fallback if activeSection somehow gets an invalid value
    ...
    const handleLoginSuccess = useCallback((receivedToken) => {
        if (receivedToken && typeof receivedToken === 'string') {
            try {
                ...
                setToken(receivedToken); 
                setAdminName('Admin'); 
                // setActiveSection('dashboard'); // No longer forcing dashboard
                fetchTodaysNotesForNotifications(receivedToken);
                fetchPendingApplicationsCount(receivedToken);
            ...
            setToken(null); setAdminName(null);
    ...
    const handleLogout = useCallback(() => {
        ...
        setTodaysNotesForNotifications([]); setShownNoteSessionIds(new Set());
        ...
        if (storedToken) {
            ...
            if (storedName) { setAdminName(storedName); }
                ...
                try {
                    ...
                    setAdminName(fullName); localStorage.setItem('admin_name', fullName);
                } catch (e) { setAdminName('Admin'); console.error("Error decoding token on initial load:", e); }
            }
            fetchTodaysNotesForNotifications(storedToken);
            fetchPendingApplicationsCount(storedToken);
        }
    }, [fetchTodaysNotesForNotifications, fetchPendingApplicationsCount]);
